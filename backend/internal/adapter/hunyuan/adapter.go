...
func (a *Adapter) ProviderName() string                               { return "hunyuan" }
func (a *Adapter) ModelName() string                                  { return a.modelName }
func (a *Adapter) Capabilities() adapter.Capabilities                 { return adapter.Capabilities{SupportsStreaming: true, SupportsToolCalling: true, MaxContextTokens: 32768, MaxOutputTokens: 4096} }
func (a *Adapter) Pricing() *adapter.ModelPricing                     { return &adapter.ModelPricing{InputPricePerToken: 0.000001, OutputPricePerToken: 0.000001, Currency: "CNY"} }
...
func (a *Adapter) ChatCompletionStream(ctx context.Context, req *adapter.ChatRequest) (<-chan *adapter.StreamChunk, error) {
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if err := json.Unmarshal([]byte(data), &chunk); err == nil { ch <- &chunk }
		}
	}()
	return ch, nil
}
...
func (a *Adapter) doRequest(ctx context.Context, req *adapter.ChatRequest, stream bool) (*adapter.ChatResponse, error) {
	...
	if resp.StatusCode != 200 {
		...
		return nil, fmt.Errorf("hunyuan api error %d: %s", resp.StatusCode, string(body))
	}
	var chatResp adapter.ChatResponse
	if err := json.NewDecoder(resp.Body).Decode(&chatResp); err != nil { return nil, fmt.Errorf("decode: %w", err) }
	return &chatResp, nil
}
