...
func New(baseURL, apiKey, modelName string) *Adapter {
	return &Adapter{
		...
		apiKey:    apiKey,
		modelName: modelName,
		httpClient: &http.Client{Timeout: 120 * time.Second, Transport: &http.Transport{MaxIdleConns: 100, MaxIdleConnsPerHost: 20, IdleConnTimeout: 90 * time.Second}},
	}
}

func (a *Adapter) ProviderName() string                                          { return "kimi" }
func (a *Adapter) ModelName() string                                             { return a.modelName }
func (a *Adapter) Capabilities() adapter.Capabilities                            { return adapter.Capabilities{SupportsStreaming: true, SupportsVision: false, SupportsToolCalling: true, MaxContextTokens: 131072, MaxOutputTokens: 8192} }
...
func (a *Adapter) HealthCheck(ctx context.Context) error                         { req, _ := http.NewRequestWithContext(ctx, "GET", a.baseURL+"/v1/models", nil); req.Header.Set("Authorization", "Bearer "+a.apiKey); resp, _ := a.httpClient.Do(req); if resp != nil { resp.Body.Close() }; return nil }
